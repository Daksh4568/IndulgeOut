...
const KYCSetupPage = () => {
  ...
  const handleFinalSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to save KYC details. Please try again.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-black flex items-center justify-center relative overflow-hidden p-4">
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        >
          ...
          {/* Heading */}
          <h1 className="text-2xl font-bold text-white text-center mb-2" style={{ fontFamily: 'Oswald, sans-serif' }}>
            Please Submit your details
          </h1>

          {/* Step Indicator */}
          <div className="flex items-center justify-center gap-4 mb-8">
            <div className={`flex items-center gap-2 ${activeStep === 1 ? 'opacity-100' : 'opacity-50'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
                activeStep === 1 ? 'bg-gradient-to-r from-[#7878E9] to-[#3D3DD4] text-white' : 'bg-white/10 text-gray-400'
              }`}>
                1
              </div>
            ...
            <div className={`flex items-center gap-2 ${activeStep === 2 ? 'opacity-100' : 'opacity-50'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
                activeStep === 2 ? 'bg-gradient-to-r from-[#7878E9] to-[#3D3DD4] text-white' : 'bg-white/10 text-gray-400'
              }`}>
                2
              </div>
          ...
          {activeStep === 1 && (
            <form onSubmit={handleStep1Submit} className="space-y-6">
              <div className="bg-white/5 border-2 border-dashed border-white/20 rounded-xl p-8 hover:border-[#7878E9] transition-all">
                ...
                >
                  ...
                  <p className="text-white font-medium mb-2">
                    {idProof ? idProof.name : 'Choose a file or drag & drop it here'}
                  </p>
              ...
              <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                <h3 className="text-white font-semibold mb-3 text-sm">Upload your Government ID proof</h3>
                <p className="text-gray-400 text-xs leading-relaxed">
                  Aadhaar card / PAN card / Passport / Voter ID card / Driving Licence
                </p>
              </div>
